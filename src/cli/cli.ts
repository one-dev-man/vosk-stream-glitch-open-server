    ...
    help: {
        ...
        callback: async (label: string, args: Array<any>, cli: CLI) => {
            if(args.length < 1) {
                ...
                else console["__log"]("Command \""+args[0]+"\" not found.");
            }
        }
    },
    // history: {
...
export class Environment {
    ...
    get firstCommandLabel() { return this.#dictionary[this.ARG_FIRST_COMMAND_LABEL_KEY] as string; }
...
export type CLI_FIRST_COMMAND_CONSTRUCTOR_TYPE = { help?: string, callback: CLI_COMMAND_CALLBACK_TYPE };
...
export class CLI extends Listener {
    ...
    #firstCommand: Command | null = null;
    ...
    setFirstCommand(command: Command | CLI_FIRST_COMMAND_CONSTRUCTOR_TYPE) {
        ...
        } : command ) as CLI_COMMAND_CONSTRUCTOR_TYPE;
        cmd_constructor.label = this.argv.firstCommandLabel;
        this.#firstCommand = new Command(cmd_constructor);

        this.registerCommand(this.#firstCommand);
    }

    removeFistCommand() {
        if(this.#firstCommand) {
            this.unregisterCommand(this.#firstCommand)
